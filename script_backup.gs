...
function sortLive() {
  ...
  if (lastRow > 1) {
    var rangeToSort = sheet.getRange(2, 1, lastRow - 1, sheet.getLastColumn());
    var values = rangeToSort.getValues();
    ...
    values.sort(function(a, b) {
      var artistA = a[1].toString().trim();
      var artistB = b[1].toString().trim();
      
      var artistAWithoutThe = artistA.startsWith("The ") ? artistA.slice(4) : artistA;
      var artistBWithoutThe = artistB.startsWith("The ") ? artistB.slice(4) : artistB;

      return artistAWithoutThe.localeCompare(artistBWithoutThe);
    });
    
    rangeToSort.offset(0, 0, values.length, sheet.getLastColumn()).setValues(values);
  }
}
*/
